...
/// Compute the points domain for points on a local node.
pub fn compute_domain(points: &Vec<[PointType; 3]>) -> Domain {

    let max_x = points.iter().max_by(|a, b| a[0].partial_cmp(&b[0]).unwrap()).unwrap()[0];
    let max_y = points.iter().max_by(|a, b| a[1].partial_cmp(&b[1]).unwrap()).unwrap()[1];
    let max_z = points.iter().max_by(|a, b| a[2].partial_cmp(&b[2]).unwrap()).unwrap()[2];

    let min_x = points.iter().min_by(|a, b| a[0].partial_cmp(&b[0]).unwrap()).unwrap()[0];
    let min_y = points.iter().min_by(|a, b| a[1].partial_cmp(&b[1]).unwrap()).unwrap()[1];
    let min_z = points.iter().min_by(|a, b| a[2].partial_cmp(&b[2]).unwrap()).unwrap()[2];
...
pub fn compute_global_domain(local_points: &Vec<[PointType; 3]>, comm: &UserCommunicator) -> Domain {

    let size = comm.size();
    let rank = comm.rank();

    let local_domain = compute_domain(local_points);
    ...
    // Find minimum origin
    let min_x = buffer.iter().min_by(|a, b| a.origin[0].partial_cmp(&b.origin[0]).unwrap()).unwrap().origin[0];
    let min_y = buffer.iter().min_by(|a, b| a.origin[1].partial_cmp(&b.origin[1]).unwrap()).unwrap().origin[1];
    ...
    let max_z = buffer.iter().max_by(|a, b| a.origin[2].partial_cmp(&b.origin[2]).unwrap()).unwrap().origin[2];

    let max_origin = [max_x, max_y, max_z];

    // Find maximum diameter (+max origin)
    let max_x = buffer.iter().max_by(|a, b| a.diameter[0].partial_cmp(&b.diameter[0]).unwrap()).unwrap().diameter[0];
    let max_y = buffer.iter().max_by(|a, b| a.diameter[1].partial_cmp(&b.diameter[1]).unwrap()).unwrap().diameter[1];
...
mod tests {
    ...
    fn test_compute_bounds() {
        ...
        let domain = compute_domain(&points);

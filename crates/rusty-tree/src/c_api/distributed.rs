...
#[no_mangle]
pub extern "C" fn distributed_tree_points_to_keys_get(p_tree: *const DistributedTree, p_point: *const Point) -> *mut MortonKey {
    let tree = unsafe {&*p_tree};
    let point = unsafe { *p_point };
    Box::into_raw(Box::new(*tree.points_to_keys.get(&point).unwrap()))
}

#[no_mangle]
pub extern "C" fn distributed_tree_keys_to_npoints_get(p_tree: *const DistributedTree, p_key: *const MortonKey) -> usize {
    let tree = unsafe {&*p_tree};
    let key = unsafe { *p_key };
    tree.keys_to_points.get(&key).unwrap().len()
}

#[no_mangle]
pub extern "C" fn distributed_tree_keys_to_points_get(p_tree: *const DistributedTree, p_key: *const MortonKey) -> *const Point {
    let tree = unsafe {&*p_tree};
    let key = unsafe { *p_key };
